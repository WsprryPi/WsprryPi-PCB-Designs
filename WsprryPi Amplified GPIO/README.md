# WsprryPi Amplified GPIO

A KiCad 10 project initialized from the `RPi Full-Size Hat` template for amplified GPIO development. It provides the official classic full-size HAT board geometry, the complete 40-pin GPIO interface, recommended display/camera flex openings, mounting-hole clear lands, independent local libraries, and a socket-header purchasing item. The schematic is intentionally unwired, and the board has no tracks, vias, or copper pours.

## Develop this HAT project

1. Open `WsprryPi Amplified GPIO.kicad_pro` in KiCad 10.
2. Add the required identification EEPROM and application circuit. Connect only the GPIOs the design uses; mark genuinely unused pins with no-connect flags.
3. Run **Tools → Update PCB from Schematic…**, place the added components, route the board, add and fill copper zones, and run ERC and DRC.

This project has independent copies of the design files, symbol library, footprint library, and STEP model. Changes here do not update `RPi Full-Size Hat`, and later template changes do not update this project.
...
This project follows the classic Raspberry Pi full-size HAT mechanical drawing, but the finished board is not automatically a compliant HAT. Before calling it a Raspberry Pi HAT, complete and verify all applicable requirements:
...
Both library tables use the `wsprrypi-amplified-gpio` nickname and `${KIPRJMOD}` paths. All required project libraries are inside this directory. H1 is a separate, selectable footprint that shows the female 2×20 HAT socket on the underside of the board. Delete H1 when the 3D socket is not wanted, or place `wsprrypi-amplified-gpio:Raspberry_Pi_HAT_2x20_Socket_3D` to add it back. H1 has no electrical pads; U1 remains the authoritative electrical and mechanical interface, and J1 remains the purchasing item.
...
- [Project settings](WsprryPi%20Amplified%20GPIO.kicad_pro): rules, defaults, and BOM preset.
- [Schematic](WsprryPi%20Amplified%20GPIO.kicad_sch): U1 GPIO interface and J1 procurement symbol.
- [PCB](WsprryPi%20Amplified%20GPIO.kicad_pcb): locked interface footprint, outline, mounting holes, and flex openings.
- [Symbol library](wsprrypi-amplified-gpio.kicad_sym): GPIO interface, socket purchasing symbol, and grouped J81/J82 connector symbol.
- [Interface footprint](wsprrypi-amplified-gpio.pretty/Raspberry_Pi_HAT_Interface.kicad_mod).
- [Selectable 2×20 socket footprint](wsprrypi-amplified-gpio.pretty/Raspberry_Pi_HAT_2x20_Socket_3D.kicad_mod): board-only 3D representation used by H1.
- [Grouped J81/J82 footprint](wsprrypi-amplified-gpio.pretty/Dual_PinSocket_1x04_P2.54mm_J81_J82.kicad_mod): exact source spacing plus inter-header copper and placement keepout.
- [2×20 socket STEP model](wsprrypi-amplified-gpio.3dshapes/PinSocket_2x20_P2.54mm_Vertical.step) and [model notice](wsprrypi-amplified-gpio.3dshapes/README.md).
...
Run ERC and DRC after adding the ID EEPROM, application circuit, routing, and copper. This project has no verified physical assembly, connector fit, HAT identity data, Raspberry Pi model compatibility, electrical performance, or RF performance.
